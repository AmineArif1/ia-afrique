(function ($) {
    ...
    // ****************************
    if ($.fn.countdown) {
        $("#clock").countdown("2022/06/29", function (event) {
            $(this).html(event.strftime("<div>%m <span>Mois</span></div> <div>%d <span>Jours</span></div> <div>%H <span>Heurs</span></div> <div>%M <span>Minutes</span></div> <div>%S <span>Seconds</span></div>"));
        });
    }
    ...
    if ($.fn.scrollUp) {
        confer_window.scrollUp({
            scrollSpeed: 1000,
            scrollText: '<i class="bi bi-caret-up-fill"></i>'
        });
    }
